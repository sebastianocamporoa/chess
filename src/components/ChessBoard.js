...
export class ChessBoard extends HTMLElement {
  ...
  // renderiza las posiciones de las celdas
  genFakeCells(n) {
    const texts = (n === 8 ? "87654321" : " abcdefgh ").split("");
    return texts.map((text) => `<div class="fake">${text}</div>`).join("");
  }

  // renderiza las filas y las columnas
  renderCells() {
  ...
  onClick(cell) {
    ...
    // devuelve falso al estar seleccionada una ficha y verdadero al cancelar la selección
  ...
  getCell(coords) {
    ...
    // renderiza el camino por donde se puede mover la ficha
    return this.shadowRoot.querySelector(`[row="${row}"][col="${col}"]`);
  }

  selectPiece(sourceCell) {
    // define cual es la ficha seleccionada
    const sourcePiece = sourceCell.piece;
    // Si la ficha seleccionada es valida (del turno del jugador correcto) pinta la celda
    ...
    if (isValidPiece) {
      ...
      // devuelve todos los movimientos y filtra por los que son posibles para la ficha
      const moves = virtualBoard
        .getAllMoves(sourceCell.coords)
        .filter((move) => move.possible);

      moves.forEach((move) => {
        const newVirtualBoard = new VirtualBoard(fen);
        // modifica el tablero con los nuevos estados de las fichas
  ...
  selectTarget(targetCell) {
    ...
    const sourcePiece = sourceCell.piece;
    // mueve la ficha si el movimiento es posible
    this.moveTo(sourcePiece, targetCell);
  }

  reset() {
    // reinicia el estilo de la celda al quitarse la seleccion de una ficha
  ...
  moveTo(sourcePiece, targetCell) {
    // valida si la ficha está atacando
    const isAttack = Boolean(targetCell.piece);
    const sourceCell = this.shadowRoot.querySelector("chess-cell.selected");
    this.reset();
    this.stage.next();

    // ejecuta animación al eliminar una ficha enemiga
  ...
  attackPiece(battleCell) {
    ...
    // quita la ficha eliminada del tablero
